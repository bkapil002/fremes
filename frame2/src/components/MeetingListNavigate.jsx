...
export default function MeetingListNavigate() {
    const { encodedEmail, linkId } = useParams();
  ...
  useEffect(() => {
    ...
    try {
      // If the encodedEmail already contains '@', redirect immediately
      if (encodedEmail.includes("@")) {
        navigate("https://community.samzara.in");
        return;
      }

      // Decode the email (adjust slicing as per your encoding logic)
    ...
    const doLogin = async () => {
      try {
        ...
        // Navigate to meeting page only if login succeeds
        navigate(`/room/${linkId}`);
        setError(null);
      } catch (err) {
        console.error(err);
    ...
    doLogin();
  }, [encodedEmail, linkId, login, navigate]);
